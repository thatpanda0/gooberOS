// Old version of the script (more stable)

let zCounter = 1; // global focus counter

document.querySelectorAll('.window').forEach(win => {

  const draggableArea = win.classList.contains('settings-window') // Settings drag
  ...
  titlebar.addEventListener('pointerdown', beginPointer('drag'));
  ...
  document.addEventListener('pointermove', e => {
    ...
    if (mode === 'drag') {
      ...
      win.style.height = Math.max( 28, startHeight + dy) + 'px';
    }
  });
  ...
  document.querySelectorAll('.dock-item').forEach(button => {
    button.addEventListener('click', () => {
      ...
      const win = document.querySelectorAll('.window')[winIndex];
...
(function(){
      ...
      function render(){
        ...
        const title = document.createElement('div'); title.className='cal-title'; title.textContent = new Date(year,month).toLocaleString(undefined,{month:'long', year:'numeric'});
        ...
        for(let t=1;t<=trailing;t++){
          const d = document.createElement('div'); d.className='day inactive'; d.innerHTML=`<div class="date">${t}</div>`; grid.appendChild(d);
        }
      ...
      content.addEventListener('wheel', (e)=>{
        if(content.scrollHeight > content.clientHeight) return; e.preventDefault();
      }, {passive:false});
